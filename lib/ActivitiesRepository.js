 ...
 * @constructor
 */
module.exports = function ActivitiesRepository(campaign, conn) {
   ...
   * @private
   */
  var SAVE_QUERY = "insert into activities (creation_time, campaign_id, " +
    "user_id, legislative_id, action) values (now(), " +
  ...
  var STATS_QUERY = "select sum(if(action = 'tweet', 1, 0)) as tweets, " +
    ...
    "sum(if(action = 'fb', 1, 0)) as facebookMessages from activities a " +
    "inner join campaigns c on a.campaign_id = c.id " +
    "where c.name = ?";
