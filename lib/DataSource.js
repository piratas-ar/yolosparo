...
module.exports = function DataSource(connectionString) {
  ...
  /** Creates the SQL insert statement for the specified model object.
   * @param {Object} entity Model object to convert. Cannot be null.
   * @return {String} The SQL statement for the specified entity.
   */
  var toSql = function (entity) {
    var values = Object.keys(entity).map(function (property) {
  ...
  /** Converts a list of objects to a SQL INSERT statement.
   ...
   *    never null.
   */
  var fromJSON = function (table, rows) {
    var statements = rows.reduce(function (prev, row, index, array) {
      var result = prev + toSql(row);
    ...
    fs.readdir(path.join(SQL_DIR, "db-setup.d"), function (err, files) {
      ...
      files.sort().forEach(function (file) {
        ...
        if (file.substr(-4) === "json") {
          jsonStatements = fromJSON(tableName, JSON.parse(data));
