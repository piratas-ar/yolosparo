...
module.exports = function LegislativesRepository(conn) {
   ...
   * @private
   */
  var ALL_QUERY = "select *, count(activities.id) as cuenta from legislatives left join activities on (legislative_id = legislatives.id) group by legislatives.id";
  ...
  return {
    ...
    list: function (filter, callback) {
      ...
      query += " order by cuenta asc, full_name";
